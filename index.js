const CONFIG = {
  ...
  baseBackoffMs: 250,
  cacheTtlMs: 120000,
  // Bounded cache: evict LRU entries when limit exceeded (prevents memory leak)
  cacheMaxEntries: 256,
  thumbnailSize: 512,
  // WEB_REMIX client version — update this periodically if search stops working
  clientVersion: "1.20250612.01.00",
  ...
  innerTubeClientVersion: "20.10.38",
  innerTubeUserAgent: "com.google.android.youtube/20.10.38 (Linux; U; Android 11) gzip",
...
const USER_AGENTS = [
  ...
  "Mozilla/5.0 (Linux; Android 15; Pixel 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.6478.122 Mobile Safari/537.36"
];
...
function _cacheEvictIfNeeded() {
  if (_cache.size <= CONFIG.cacheMaxEntries) return;
  // Delete oldest entries (Map preserves insertion order)
  var toDelete = _cache.size - CONFIG.cacheMaxEntries;
...
function cacheGet(k) {
  ...
  // Move to end (LRU: most recently accessed stays)
  _cache.delete(k);
  _cache.set(k, e);
  return e.v;
}
function cacheSet(k, v) {
  _cache.delete(k); // ensure insertion at end
...
function normalizeUrl(u) {
  ...
  try {
    ...
    if (Array.isArray(CONFIG.allowlistHosts) && CONFIG.allowlistHosts.length > 0) {
      if (CONFIG.allowlistHosts.indexOf(parsed.hostname) === -1) {
        L("warn", "normalizeUrl blocked non-allowlisted host:", parsed.hostname);
        return null;
      }
    }
...
var INNERTUBE_CLIENTS = [
  {
    ...
    ua: "com.google.android.apps.youtube.vr.oculus/1.67.10 (Linux; U; Android 12L; eureka-user Build/SQ3A.220605.009.A1) gzip",
    ...
    body: {
      context: {
        client: {
          ...
          timeZone: "UTC",
          utcOffsetMinutes: 0,
          userAgent: "Mozilla/5.0 (iPad; CPU OS 18_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.4 Mobile/15E148 Safari/604.1,gzip(gfe)"
        }
      }
    },
    ua: "Mozilla/5.0 (iPad; CPU OS 18_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.4 Mobile/15E148 Safari/604.1,gzip(gfe)",
    key: CONFIG.innerTubeApiKey
  },
  {
    name: "android",
    ...
    body: {
      context: {
        client: {
          ...
          timeZone: "UTC",
          utcOffsetMinutes: 0,
          osName: "iOS", osVersion: "18.4.1",
          platform: "MOBILE"
        }
      }
    },
    ua: "com.google.ios.youtube/20.10.4 (iPhone17,3; U; CPU iOS 18_4_1 like Mac OS X;)",
...
function buildYouTubeChallengeSolver(playerJS) {
  ...
  try {
    // SECURITY NOTE: We eval YouTube's own player JS to extract the signature
    // transform function. The trust model here is: we already fetched the
    // YouTube watch page over HTTPS (pinned cert on most platforms), and the
    // player JS URL comes from that same page. If the transport is
    // compromised, we have bigger problems. The fakeWindow/fakeDocument stubs
    // above isolate the eval from the real DOM/XHR environment.
...
function isDownloadAuthFailure(result) {
  ...
  return text.indexOf("403") >= 0 ||
         ...
         text.indexOf("http 401") >= 0 ||
         text.indexOf("range not satisfiable") >= 0 ||
...
registerExtension({
  ...
  checkAvailability: function(isrc, trackName, artistName, options) {
    ...
    // A Spotify ID matches the YouTube video ID format (11 chars base64url),
    // but only trust it if it's explicitly a known YouTube video ID shape.
    // We add an extra heuristic: Spotify IDs often start with digits or
    // specific patterns, while YouTube IDs are more varied. Since we can't
    // perfectly disambiguate, we accept the shortcut only when poTokenMode
    // is off (no auth context that would expose us to false lookups).
    if (spotifyId && /^[A-Za-z0-9_-]{11}$/.test(spotifyId) && CONFIG.poTokenMode === "off") {
